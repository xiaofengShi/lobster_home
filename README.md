<p align="center">
  <img src="images/hero.jpg" alt="LobsterHive" width="100%">
</p>

<h1 align="center">🦞🐝 LobsterHive</h1>
<p align="center"><strong>蜂群智能守护家庭 — Swarm Intelligence meets Smart Home</strong></p>
<p align="center">一群 AI 蜜蜂守护一个家，一只龙虾统筹一切。</p>

<p align="center">
  <a href="https://xiaofengshi.github.io/lobster_home/">🌐 在线主页</a> ·
  <a href="https://xiaofengshi.github.io/lobster_home/#dashboard">📊 实时仪表盘</a> ·
  <a href="https://feishu.cn/docx/QLuUd9qvvoGDHmx2JMPcnZhKn3c">📄 项目文档</a>
</p>

---

## 它是什么

**LobsterHive** 是一个基于蜂群智能（Swarm Intelligence）的多 Agent 智能家居系统。不是一个大模型做所有事，而是 **5 只专业蜜蜂 + 1 只蜂后**，各司其职守护你的家。

> 蜜蜂用摇摆舞传递食物方向和距离 → 我们用 JSON 事件传递结构化数据。

## 🐝 蜂群矩阵

| 蜜蜂 | 角色 | 职责 |
|------|------|------|
| 👑🦞 **蜂后 Alan** | Queen | 总调度——不干活，只指挥 |
| 🔭 **侦查蜂** | Scout | 摄像头截图 + VLM 视觉分析 + 传感器 + 天气 |
| ⚔️ **守卫蜂** | Guard | 纯规则引擎安全检测（摔倒/火灾/入侵） |
| 💊 **哺育蜂** | Nurse | 个性化天气关怀、课前提醒、到家通知 |
| 💃 **舞蹈蜂** | Dancer | 统一通知出口（飞书 + 音箱），去重 + 静音 |
| 📖 **筑巢蜂** | Builder | 指纹学习、行为统计、知识持久化、反馈分析 |
| 👁️‍🗨️ **侦察守卫蜂** | ScoutGuard | 感知+安全一体化（可选，合并模式） |

**日成本 ≈ ¥4.35** — 只有"看"需要大模型，其他全是规则引擎或轻量推理。

## ⚡ 核心亮点

### 🎯 摇摆舞 = 事件总线

蜜蜂之间通过结构化 JSON 事件通信，完全解耦：

```
门锁触发 → 侦查蜂发布 door_unlock 事件
         → 守卫蜂：已知指纹 + 合理时段 → ✅ 安全
         → 哺育蜂：17:17 = 接孩子回来 → 关怀消息
         → 舞蹈蜂：飞书 "孩子到家啦！🎒" + 音箱播报
         → 筑巢蜂：记录到家时间，学习行为模式
```

### 🔭 两级视觉策略

帧差检测跳过无变化画面，仅在画面变化或传感器触发时调用 VLM。**节省 60-80% VLM 调用**。

### 🛡️ 双层安全检测

守卫蜂第一层**关键词匹配**（<1ms），第二层 **n-gram 语义相似度**兜底。关键词漏检时语义层兜住。

- ⏱️ **<1ms** 延迟 · 💰 **¥0** 成本 · 🎯 **双层兜底**

### 🧬 贝叶斯指纹学习

VLM 视觉特征 + 门锁指纹 ID 交叉验证。**贝叶斯后验概率 > 90%** 自动确认身份。

### ⚡ 并行编排

蜂后使用 ThreadPoolExecutor 并行调度：异常检测 ∥ 早间播报，日报 ∥ 到家 ∥ 晚间。

### 🏥 蜂巢自愈 + Dead-Letter

蜜蜂连续失败 → 自动重置 + 通知管理员。事件消费失败 → dead-letter 记录，不丢事件。

### 🔻 四级优雅降级链 (NEW)

```
VLM 挂了 → 侦查蜂降级为"纯传感器模式"（只读门锁/温湿度）
LLM 挂了 → 哺育蜂降级为"模板模式"（固定关怀文案）
飞书挂了 → 舞蹈蜂降级为"音箱 only"→ 再挂 → 纯日志
全挂了   → 蜂后进入"生存模式"（纯规则引擎处理紧急事件）
```

### 🎯 三级自动化 (NEW)

置信度评估决定自动化级别：

| 级别 | 条件 | 动作 |
|------|------|------|
| 🟢 全自动 | 置信度 >90% | 直接执行 |
| 🟡 半自动 | 50-90% | 执行+通知确认 |
| 🔴 需确认 | <50% | 只通知，等确认 |

### 📊 反馈回路 (NEW)

筑巢蜂每周分析通知效果：高频低价值通知 → 建议降频，误报率 >15% → 建议调规则。系统越用越聪明。

## 📊 关键指标

| 指标 | 数值 |
|------|------|
| 总代码行数 | **~5600+** |
| 自动化测试 | **35 项** |
| VLM 调用节省 | **60-80%** |
| 日运行成本 | **~¥1.5** |
| 安全检测延迟 | **<1ms** |
| 贝叶斯确认阈值 | **>90%** |
| 降级链层级 | **4 级** |
| 置信度评估 | **3 级自动化** |

## 🏠 实际场景

**🌅 晨间播报** — 起床后自动播天气、穿衣建议、今日课程

**📬 口述提醒** — 说一句"明天下午3点周会"，到点双通道提醒

**👶 课前提醒** — 提前 30 分钟自动提醒准备出门

**🔑 到家检测** — 门锁指纹识别，个性化欢迎

**🌙 晚间收尾** — 检查门锁、灯光、温度，生成活动总结

**⛈️ 暴雨联动** — 天气预警自动触发关怀 + 调整策略

## 🔒 隐私承诺

| 规则 | 说明 |
|------|------|
| 🌙 20:00后摄像头休眠 | 家人隐私保护，不可覆盖 |
| 🔐 用户关闭 = 绝对不开 | AI 不自行重启任何被关闭的设备 |
| 🏠 家人在家 = 永远正常 | 不当考勤机，不发异常告警 |
| 🤫 22:00后全系统静音 | 紧急事件除外 |

## 🏗️ 架构

```
┌──────────────────────────────────────────────┐
│              👑 蜂后 Queen                    │
│   Handoff 直接调用链 + 并行编排 + 自愈 + 周报  │
└───────┬─────┬─────┬─────┬─────┬─────────────┘
        │     │     │     │     │
   ┌────▼──┐ ┌▼───┐ ┌▼──┐ ┌▼──┐ ┌▼────┐
   │🔭Scout│ │⚔️  │ │💊 │ │💃 │ │📖   │
   │帧差+  │ │双层 │ │关怀│ │通知│ │贝叶 │
   │VLM    │ │检测 │ │推理│ │追踪│ │斯学习│
   │+降级  │ │+置信│ │+模板│ │+降级│ │+反馈│
   └───┬───┘ └──┬──┘ └─┬─┘ └─┬─┘ └──┬──┘
       │        │      │     │      │
   ┌───▼────────▼──────▼─────▼──────▼───┐
   │  💃 摇摆舞事件总线（审计日志模式）    │
   │  主路径：直接函数调用链 (Handoff)     │
   └─────────────────────────────────────┘
```

## 技术栈

| 层 | 技术 |
|----|------|
| AI 引擎 | [OpenClaw](https://openclaw.ai) |
| 视觉理解 | 大型视觉语言模型 (VLM) |
| 安全检测 | 纯规则引擎 |
| 智能家居 | Home Assistant + 米家 |
| 通知 | 飞书 + 智能音箱 |
| 语音 | TTS + Opus 编码 |
| 可靠性 | JSONL 轮转 · Dead-Letter · 线程安全 · 35项自动测试 |

---

<p align="center">
  <em>让技术有人情味，这就是 LobsterHive 的核心竞争力。</em>
</p>
<p align="center">
  <em>Powered by <a href="https://openclaw.ai">OpenClaw</a> · Built with 🦞 & ❤️</em>
</p>
