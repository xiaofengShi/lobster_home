...
| 🔭 **侦查蜂** | Scout | 摄像头截图 + VLM 视觉分析 + 传感器 + 天气 | `qwen3-vl-235b-a22b-instruct` |
...
| 视觉理解 | `qwen3-vl-235b-a22b-instruct` (Qwen3-VL-235B, 22B 激活) |
| 关怀/编排 LLM | `claude-haiku` (轻量推理) |
| 安全检测 | 纯规则引擎 (零模型依赖) |
| 智能家居 | Home Assistant + 米家 |
| 通知 | 飞书 API + 小爱音箱 TTS |
| 语音 | Edge-TTS (`zh-CN-YunxiaNeural`) + Opus 编码 |
...
| 模型 | 全名 | 用途 | 调用频率 |
|------|------|------|---------|
| **VLM** | `qwen3-vl-235b-a22b-instruct` | 摄像头截图理解、人员识别 | 每5分钟 |
| **LLM** | `claude-haiku` | 天气关怀生成、晚间总结 | 按需 |
| **TTS** | `zh-CN-YunxiaNeural` (Edge-TTS) | 音箱中文播报 | 按需 |
